...
#define SERVAPP_NUM_ATTR_SUPPORTED        25//21
...
static uint8 simpleProfileChar7Props = GATT_PROP_NOTIFY | GATT_PROP_READ | GATT_PROP_WRITE ;
...
static gattAttribute_t simpleProfileAttrTbl[SERVAPP_NUM_ATTR_SUPPORTED] = 
{
  ...
  { 
    ...
    { 
      ...
      // Characteristic 7 User Description 24
      { 
        { ATT_BT_UUID_SIZE, charUserDescUUID },
        GATT_PERMIT_READ, 
        0, 
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    switch ( uuid )
    {
      ...
      case GATT_CLIENT_CHAR_CFG_UUID:
        ...
        //CHAR6的通知开关        
        if(pAttr->handle == simpleProfileAttrTbl[GUA_ATTRTBL_CHAR6_CCC_IDX].handle) {   
          status = GATTServApp_ProcessCCCWriteReq( connHandle, pAttr, pValue, len,  
                                                   offset, GATT_CLIENT_CFG_NOTIFY );  
        } 
        //CHAR7的通知开关        
...
// 发送数据到主机
void simpleBLE_SendData( uint16 nGUA_ConnHandle, uint8* buffer, uint8 sendBytes)
{
  ...
  if (nGUA_Return & GATT_CLIENT_CFG_NOTIFY){
    ...
    pReport.handle = simpleProfileAttrTbl[GUA_ATTRTBL_CHAR7_VALUE_IDX].handle; ; 
    osal_memcpy(pReport.value, buffer, sendBytes);
    GATT_Notification( nGUA_ConnHandle, &pReport, FALSE );            
  }
}
