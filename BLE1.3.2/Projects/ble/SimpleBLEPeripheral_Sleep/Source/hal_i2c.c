...
i2cLen_t HalI2CReadReg_2(uint8 address, uint8 Regaddress, i2cLen_t len, uint8 *pBuf)
{
  uint8 cnt = 0;
  uint8 pBuf_temp[1];
  //HalI2CInit( i2cClock_123KHZ );
  I2C_Init();
  I2C_ReadHaveAdd(pBuf_temp,len,0x00,0x00);
  pBuf_temp[0] = 0x00;
  if(I2C_ReadHaveAdd(pBuf,len,Regaddress,address) == TRUE)
  {
    cnt = len;
  }
  HalI2CInit( i2cClock_123KHZ );
...
i2cLen_t HalI2CReadReg_3(uint8 address, uint8 Regaddress, i2cLen_t len, uint8 *pBuf)
{
 /* switch(Delay_Count)
  {
    case 0:
...
i2cLen_t HalI2CReadReg(uint8 address, uint8 Regaddress, i2cLen_t len, uint8 *pBuf)
{
  uint8 cnt = 0;
  HalI2CInit( i2cClock_123KHZ );
  I2C_STRT();
...
i2cLen_t HalI2CWriteReg(uint8 address, uint8 Regaddress, i2cLen_t len, uint8 *pBuf)
{
/*   if (i2cMstStrt(address, 0) != mstAddrAckW)
  {
    len = 0;
...
*/
i2cLen_t HalI2CMax30102ReadFifo(uint8 address, uint8 Regaddress, uint32 *pun_red_led, uint32 *pun_ir_led)
{
  uint32 un_temp;
 ...
 // I2C_STRT();
  ...
  I2C_READ(un_temp); //un_temp = IIC_Read_Byte(0);//
  I2C_SET_ACK();
  un_temp<<=16;
  *pun_red_led+=un_temp;
